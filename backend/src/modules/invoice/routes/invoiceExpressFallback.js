...
const handleInvoiceExpressFallback = (req, res) => {
  ...
  if (path.includes('/upload')) {
    ...
    const newDoc = {
      ...
      created_at: new Date().toISOString(),
      extracted_data: JSON.parse(JSON.stringify(inMemoryDocuments[0].extracted_data))
    };
    newDoc.extracted_data.invoice_details.invoice_number = `INV/${new Date().getFullYear()}/${Math.floor(1000 + Math.random() * 9000)}`;
    newDoc.extracted_data.invoice_details.invoice_date = new Date().toISOString().split('T')[0];
